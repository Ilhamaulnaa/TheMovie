package com.ilham.event.movie_detail.data.mapper_impl

import com.ilham.event.common.data.ApiMapper
import com.ilham.event.movie_detail.data.remote.models.CastDto
import com.ilham.event.movie_detail.data.remote.models.MovieDetailDto
import com.ilham.event.movie_detail.domain.models.Cast
...
class MovieDetailMapperImpl : ApiMapper<MovieDetail, MovieDetailDto> {
    ...
    private fun formatTimeStamp(pattern: String = "dd.MM.yy", time: String): String {
        return try {
            if (time.isBlank() || time == "0") return "Unknown date"
            ...
            date?.let { outputDateFormatter.format(it) } ?: "Unknown date"
        } catch (e: Exception) {
            "Unknown date"
        }
    }
